...
public class RedisCacheManager {
     ...
     */
    @SuppressWarnings("finally")
    public boolean hsetobj(String hkey, String key, Object value) {
        Jedis jedis = null;
        try {
            // 从池中获取一个jedis实例
            jedis = jedisPool.getResource();
            jedis.hset(hkey.getBytes(), key.getBytes(), serialize(value));
        } catch (Exception e) {
            Log.error("Redis link Exception： ", e);
            // 销毁对象
            jedisPool.returnBrokenResource(jedis);
     ...
     */
    @SuppressWarnings("finally")
    public Object hgetobj(String hkey, String key) {
        Jedis jedis = null;
        Object ret = null;
        try {
            // 从池中获取一个jedis实例
            jedis = jedisPool.getResource();
            ret = deserialize(jedis.hget(hkey.getBytes(), key.getBytes()));
        } catch (Exception e) {
            Log.error("Redis link Exception： ", e);
            // 销毁对象
            jedisPool.returnBrokenResource(jedis);
     ...
     */
    @SuppressWarnings("finally")
    public List<Object> hgetmobj(String hkey, String[] keys) {
        Jedis jedis = null;
        ...
        try {
            ...
            for (int i = 0; i < keys.length; i++) {
                key[i] = keys[i].getBytes();
            }

            List<byte[]> objList = jedis.hmget(hkey.getBytes(), key);
     ...
     */
    @SuppressWarnings("finally")
    public boolean hdelobj(String hkey, String key) {
        Jedis jedis = null;
        try {
            // 从池中获取一个jedis实例
            jedis = jedisPool.getResource();
            jedis.hdel(hkey.getBytes(), key.getBytes());
        } catch (Exception e) {
            Log.error("Redis link Exception： ", e);
            // 销毁对象
            jedisPool.returnBrokenResource(jedis);
